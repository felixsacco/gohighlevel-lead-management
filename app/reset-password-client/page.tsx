...
export default function ResetPasswordClientPage() {
  ...
  const handlePasswordReset = async (e: React.FormEvent) => {
    ...
    try {
      // Update password in clients table ONLY
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <div className="flex items-center justify-center mb-4">
            ...
            <CardTitle className="font-bold tracking-tight" style={{ fontWeight: 700 }}>Reset Client Password</CardTitle>
          </div>
          <p className="text-gray-600">
            {step === 1 
              ? "Enter your email address to verify your client account." 
          ...
          {step === 1 ? (
            <form onSubmit={handleEmailVerification} className="space-y-4">
              ...
              <Button type="submit" disabled={isLoading} className="w-full bg-brand-amber hover:bg-brand-amberDark text-black font-semibold" style={{ fontWeight: 800 }}>
                {isLoading ? "Verifying..." : "Verify Client Email"}
              </Button>
            ...
            <form onSubmit={handlePasswordReset} className="space-y-4">
              ...
              <Button type="submit" disabled={isLoading} className="w-full bg-brand-amber hover:bg-brand-amberDark text-black font-semibold" style={{ fontWeight: 800 }}>
                {isLoading ? "Updating..." : "Update Client Password"}
              </Button>
