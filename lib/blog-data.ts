/**
 * Blog Content Engine — single source of truth.
 *
 * Shared by `app/sitemap.ts`, `app/blog/page.tsx` (listing) and
 * `app/blog/[slug]/page.tsx` (detail) so the sitemap, listing and detail
 * pages can never drift out of sync and emit 404s or stale slugs.
 *
 * Dates are authoritative content dates (`publishedDate`), never build-time
 * stamps. If a post is revised, update `updatedAt`; otherwise it mirrors
 * `publishedDate`.
 */
